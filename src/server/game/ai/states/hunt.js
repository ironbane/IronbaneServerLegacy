...
var ExploreAndLookForEnemies = State.extend({
    ...
    execute: function(unit, dTime) {
        ...
        if (this.walkBackTimeout > 0) {
            this.walkBackTimeout -= dTime;
        }

        if (this.minimumExploreTime > 0) {
            this.minimumExploreTime -= dTime;
        }

        if (this.changeToNewPositionTimeout > 0) {
            this.changeToNewPositionTimeout -= dTime;
        }
        ...
        if (unit.template.spawnguardradius > 0) {
            if (VectorDistance(unit.startPosition, unit.position) > unit.template.spawnguardradius) {
                this.walkBackTimeout = 3.0;
            }
...
module.exports = ExploreAndLookForEnemies;
