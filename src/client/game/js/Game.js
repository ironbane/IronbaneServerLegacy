...
var Game = Class.extend({
    ...
    Tick: function(dTime) {
        ...
        if (!socketHandler.loggedIn && !cinema.IsPlaying()) {
            ...
            self.camera.lookAt(previewLocation);
        }
        ...
        if (socketHandler.loggedIn) {
            ...
            if (!this.player) {
                ...
                if (terrainHandler.status === terrainHandlerStatusEnum.LOADED && !terrainHandler.IsLoadingCells()) {
                    self.player = new Player(socketHandler.spawnLocation, new THREE.Vector3(0, socketHandler.spawnRotation, 0), socketHandler.playerData.id, socketHandler.playerData.name);
                    self.unitList.push(self.player);
                }
            }
        }
        ...
        if (terrainHandler.status === terrainHandlerStatusEnum.LOADED && !terrainHandler.IsLoadingCells() && soundHandler.loadedMainMenuMusic && !self.showingGame) {
