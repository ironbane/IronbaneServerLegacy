...
var cellLoadRange = ZoneConstants.cellSize + 16;
        ...
        this.Awake = function() {
            ...
            var buildWaterMesh = function() {
                ...
                if (!ZoneConstants.getZoneConfig('enableFluid')) return;



                var texture = TextureHandler.getTexture('images/tiles/' + ZoneConstants.getZoneConfig('fluidTexture') + '.png', true);
                texture.wrapS = THREE.RepeatWrapping;
                texture.wrapT = THREE.RepeatWrapping;
                texture.repeat.x = 1000;
                texture.repeat.y = 1000;

                var texture2 = TextureHandler.getTexture('images/tiles/' + ZoneConstants.getZoneConfig('fluidTextureGlow') + '.png', true);
                ...
                var shaderMaterial = new THREE.ShaderMaterial({
                    uniforms: uniforms,
                    vertexShader: $('#vertex_' + ZoneConstants.getZoneConfig("fluidType")).text(),
                    fragmentShader: $('#fragment_' + ZoneConstants.getZoneConfig("fluidType")).text(),
                    transparent: ZoneConstants.getZoneConfig("fluidType") === "lava" ? false : true
                    //alphaTest: 0.5
                });
                ...
                me.waterMesh.position.y = ZoneConstants.getZoneConfig('fluidLevel');
            ...
            if (ZoneConstants.getZoneConfig("enableClouds")) {
                ParticleHandler.AddCloud(ParticleTypeEnum.CLOUD, {});
            }
        ...
        this.rayTest = function(ray, options) {
            ...
            var noTerrain = angular.isUndefined(options.noTerrain) ?
                false : options.noTerrain;
            var noMeshes = angular.isUndefined(options.noMeshes) ?
                false : options.noMeshes;
            var allowBillboards = angular.isUndefined(options.allowBillboards) ?
                false : options.allowBillboards;
            var extraRange = angular.isUndefined(options.extraRange) ?
                1.0 : options.extraRange;
            var reverseRaySortOrder = angular.isUndefined(options.reverseRaySortOrder) ?
                false : options.reverseRaySortOrder;

            var unitReference = angular.isUndefined(options.unitReference) ?
            ...
            if (!noMeshes) {
                for (var m = 0; m < meshList.length; m++) {
                    ...
                    angular.forEach(subIntersects, function(i) {
                        if (i.object.unit) {
        ...
        this.RebuildOctree = function() {
            this.lastOctreeBuildPosition = this.GetReferenceLocation();
            this.octreeResults = this.skybox.terrainOctree
        ...
        this.tick = function(dTime) {
            ...
            if (this.waterMesh) {
                ...
                if (this.skybox) {
                    if (ZoneConstants.getZoneConfig("fluidType") === "lava") {
