...
IronbaneApp.factory("Skybox", ["PhysicsObject","$injector","ZoneConstants","TextureHandler","MeshHandler",function(PhysicsObject, $injector,ZoneConstants,TextureHandler, MeshHandler){
  var Skybox = function(onReady){
    var TerrainHandler = $injector.get('TerrainHandler'); // Here is your tag
    ...
    var material = new THREE.ShaderMaterial({
      ...
      fragmentShader : $('#fragment_skybox_'+ZoneConstants.getZoneConfig("skyboxShader")).text()
    });
    ...
    if ( zones[ZoneConstants.zone].type == ZoneTypeEnum.DUNGEON ) {
    ...
    //if ( zones[terrainHandler.zone]['type'] == ZoneTypeEnum.WORLD ) {
      var model = skyboxPath + ZoneConstants.zone+".js";
...
var TerrainHandler = $injector.get('TerrainHandler'); // Here is your tag
  ...
  Skybox.prototype.tick = function(dTime) {
var TerrainHandler = $injector.get('TerrainHandler'); // Here is your tag
    ...
    if ( this.sunMesh ) {
      ...
      if ( (showEditor && levelEditor.editorGUI.chForceDay)
        || ZoneConstants.getZoneConfig("lightSystem") === LightSystemEnum.DAYONLY ) {
        this.sunVector.set(0,1,0);
      }
      else if ( (showEditor && levelEditor.editorGUI.chForceNight)
        || ZoneConstants.getZoneConfig("lightSystem") === LightSystemEnum.NIGHTONLY
        || ZoneConstants.getZoneConfig("lightSystem") === LightSystemEnum.DUNGEON ) {
        this.sunVector.set(0,-1,0);
      }
      else {
      ...
      if (ZoneConstants.getZoneConfig("lightSystem") === LightSystemEnum.DUNGEON ) {
