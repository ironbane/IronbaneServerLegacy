...
IronbaneApp.factory('Unit', ["PhysicsObject","$injector", function(PhysicsObject, $injector){
  var Unit = function(position, rotation, id, name, param, size) {
    ...
    if ( angular.isDefined(this.overrideName) ) this.name = this.overrideName;
    ...
    this.drawNameMesh = angular.isDefined(this.drawNameMesh) ? this.drawNameMesh : false;

    this.enableShadow = angular.isDefined(this.enableShadow) ? this.enableShadow : true;
  ...
  Unit.prototype.getDirectionSpriteIndex= function() {
  ...
  Unit.prototype.isMainPlayer= function() {
    return ironbane.player === this;
  };
  Unit.prototype.renderNameMesh= function(name) {
  ...
  Unit.prototype.addShadow= function() {
  ...
  Unit.prototype.fullDestroy= function() {
    ironbane.getUnitList().removeUnit(this);
    this.Destroy();
  };
  Unit.prototype.Destroy= function() {

    angular.forEach(this.lightsToMaintain, function(light){
    ...
    angular.forEach(this.particleEmittersToMaintain, function(pEmitter) {
      pEmitter.removeNextTick = true;
    });

    PhysicsObject.prototype.Destroy.apply();

  };
  Unit.prototype.displayUVFrame= function(indexH, indexV, numberOfSpritesH, numberOfSpritesV, mirror) {
        if (!this.mesh) {
            return;
        }
  ...
  Unit.prototype.tick = function(dTime) {

    angular.forEach(this.timers, function(value, timer) {
      if(value>0){
    ...
    PhysicsObject.prototype.tick.apply(this, arguments);
  ...
  Unit.prototype.inRangeOfUnit = function(unit, range) {
    return this.InRangeOfPosition(unit.position, range);
  };
  var InRangeOfPosition = function(position, range) {
    return position.clone().sub(this.position).lengthSq() < range*range;
  };
  Unit.prototype.rotateTowardsTargetPosition = function(dTime) {
    var side = true;
