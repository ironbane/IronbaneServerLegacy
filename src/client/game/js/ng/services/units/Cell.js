...
IronbaneApp.factory("Cell", ["ZoneConstants", "$injector","Mesh","UnitList","MeshHandler",function(ZoneConstants, $injector, Mesh, UnitList, MeshHandler){
    var Cell = function(cellX, cellZ) {
    ...
    Cell.prototype.tick = function(dTime) {
    ...
    Cell.prototype.Load = function() {
        ...
        var objectsFile = 'data/'+ZoneConstants.zone+'/'+this.cellX+'/'+this.cellZ+'/objects.json?'+(new Date()).getTime();
        ...
        if ( isEditor ) {
          this.filesToLoad++;
          var graphFile = 'data/'+ZoneConstants.zone+'/'+this.cellX+'/'+this.cellZ+'/graph.json?'+(new Date()).getTime();
    ...
    Cell.prototype.FinishLoad = function() {
        if ( !this.filesToLoad ) {
            // Make the mesh
            this.LoadObjects();
        }
    };
    Cell.prototype.AddMesh = function() {
...
var TerrainHandler = $injector.get('TerrainHandler'); // Here is your tag
    ...
    Cell.prototype.Destroy = function() {
        ...
        _.each(this.objects, function(object) {
            ...
            UnitList.removeUnit(object);
        });

        this.objects = [];
        this.status = cellStatusEnum.DESTROYED;
    };
    Cell.prototype.ReloadWaypointsOnly = function() {
    ...
    Cell.prototype.ReloadObjectsOnly = function() {

        for(var o=0;o<this.objects.length;o++) {
            this.objects[o].Destroy();
            // Remove from unitList
        }
        UnitList.removeUnits(this.objects);
