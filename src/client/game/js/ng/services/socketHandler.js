...
IronbaneApp.service("socketHandler", function(){
        console.log("Starting socketHandler service");
    ...
    this.Connect = function(abortConnect) {
        ...
        this.socket.emit('connectServer', data, function(reply) {
            ...
            this.spawnLocation = ConvertVector3(reply.position);
            this.spawnRotation = reply.rotY;
    ...
    this.Setup = function() {
        ...
        this.socket.on('snapshot', function(snapshot) {
            //if ( !socketHandler.loggedIn ) return;
            this.bytesReceived += 2 * snapshot.length;
            for (var x = 0; x < snapshot.length; x++) {
                ...
                if (unit) {
                    if (unit != ironbane.player) {
                        ...
                        if (angular.isDefined(unitdata.rx)) unit.targetRotation.x = unitdata.rx;
                        if (angular.isDefined(unitdata.ry)) unit.targetRotation.y = unitdata.ry;
                        if (angular.isDefined(unitdata.rz)) unit.targetRotation.z = unitdata.rz;
