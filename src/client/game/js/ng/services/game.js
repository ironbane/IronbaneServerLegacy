IronbaneApp
    .factory('Game', ['$log', '$window', '$http', '$timeout', '$filter', 'TimerService', '$state',
        function($log, $window, $http, $timeout, $filter, TimerService, $state) { // using $window to reveal the globals
            ...
            Game.prototype.tick = function(dTime) {
                ...
                if ($window.socketHandler.loggedIn) {
                    ...
                    if (!game.player) {
                        if ($window.terrainHandler.status === $window.terrainHandlerStatusEnum.LOADED && !$window.terrainHandler.IsLoadingCells()) {
