...
THREE.EditorControls = function ( object, domElement ) {
	...
	var center = object.lookAtPosition.clone() || new THREE.Vector3();
...
//            target.geometry.
        ...
        var vec = object.lookAtPosition.clone().sub(object.position);

        var lengthTotal = vec.length();

        vec.normalize().multiplyScalar(target.geometry.boundingBox.size.length()+10);

        var lengthShort = vec.length();

        var newLength = lengthTotal - lengthShort;

        vec.normalize().multiplyScalar(newLength);
	...
	function onMouseWheel( event ) {
		...
		// Don't zoom if we are currently rotating an object holding the shift or alt key
		if ( keyTracker[16] || keyTracker[18] ) return;
